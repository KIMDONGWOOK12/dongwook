...
declare module 'vscode' {
	...
	/**
	 * Namespace for Antigravity extensibility APIs that allow third-party extensions
	 * to use Antigravity-specific features.
	 */
	export namespace antigravityExtensibility {
		/**
		 * Register an interceptor that can modify notebook cell code before kernel execution.
		 * The user's cell text is not modified — only the code sent to the kernel is affected.
		 *
		 * When multiple interceptors are registered, they are called in registration order,
		 * each receiving the (possibly modified) output of the previous interceptor.
		 *
		 * @param interceptor The interceptor to register
		 * @returns A disposable that unregisters the interceptor
		 */
		export function registerCellExecutionInterceptor(
			interceptor: NotebookCellExecutionInterceptor
		): Disposable;
	}

	/**
	 * An interceptor that can modify notebook cell code before kernel execution.
	 */
	export interface NotebookCellExecutionInterceptor {
		/**
		 * Called before notebook cells are executed by the kernel. The interceptor receives
		 * each cell's current code and can return modified code to be executed instead.
		 *
		 * @param cells The cells about to be executed, with their current code
		 * @param notebook The notebook document containing the cells
		 * @param token A cancellation token
		 * @returns An array of results, one per cell. Return `undefined` to pass through unchanged.
		 */
		interceptExecution(
			cells: NotebookCellExecutionInterceptorContext[],
			notebook: NotebookDocument,
			token: CancellationToken
		): ProviderResult<NotebookCellExecutionInterceptorResult[]>;
	}

	/**
	 * Context provided to a {@link NotebookCellExecutionInterceptor} for each cell being executed.
	 */
	export interface NotebookCellExecutionInterceptorContext {
		/** The cell about to be executed */
		readonly cell: NotebookCell;
		/** The current code that will be executed (may already be modified by a prior interceptor) */
		readonly code: string;
		/** The language ID of the cell */
		readonly languageId: string;
	}

	/**
	 * The result of a {@link NotebookCellExecutionInterceptor} for a single cell.
	 */
	export interface NotebookCellExecutionInterceptorResult {
		/** The (possibly modified) code to execute. If `undefined`, the original code is used. */
		readonly code?: string;
	}
}
